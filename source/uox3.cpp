...
//o------------------------------------------------------------------------------------------------o
// Prototypes for admin terminal 
void terminalCleanup(bool& isConnected, HANDLE& hPipe);
void killTerminal(PROCESS_INFORMATION& pi);
//o------------------------------------------------------------------------------------------------o
...
auto main(SI32 argc, char* argv[]) -> int
{
	...
	//o------------------------------------------------------------------------------------------------o
	// Terminal code block to startup the process and create the pipe for communication
	bool isConnected = false;
	// Get the server's working directory
	char workingDirectory[MAX_PATH];
	GetCurrentDirectoryA(MAX_PATH, workingDirectory);
	// Start the terminal.exe process from the working directory
	std::string command = std::string(workingDirectory) + "\\terminal.exe";
	...
	if (!CreateProcessA(
		nullptr,                    // Executable path (use nullptr to use command line)
		...
		std::cout << "Failed to start the terminal application." << std::endl;
	}
	std::cout << "Terminal application started successfully." << std::endl;

	// Create the named pipe server
	HANDLE hPipe = CreateNamedPipeA(
		"\\\\.\\pipe\\MyPipe",          // Pipe name
		PIPE_ACCESS_DUPLEX,             // Pipe access mode (bidirectional)
		PIPE_TYPE_BYTE | PIPE_NOWAIT,   // Pipe mode (byte-oriented, non blocking)
		PIPE_UNLIMITED_INSTANCES,       // Number of instances
		0,                              // Output buffer size
		0,                              // Input buffer size
		0,                              // Default timeout (0 = no timeout)
		nullptr                         // Security attributes
	);

	if (hPipe == INVALID_HANDLE_VALUE || hPipe == 0)
	{
		std::cout << "Terminal pipe creation failure. Error code: " << GetLastError() << std::endl;
	}
	...
	{
		std::cout << "Terminal connected." << std::endl;
		std::cout << "Waiting for the terminal to connect..." << std::endl;
		std::string command;
	}
	//o------------------------------------------------------------------------------------------------o
	// END terminal code block for initializing process and communication
	//o------------------------------------------------------------------------------------------------o
	...
	while (cwmWorldState->GetKeepRun())
	{
		...
		// hook terminal into main loop BEGIN
		SI32 toLoad = -1; // for reload command
		...
		if (ReadFile(hPipe, buffer, sizeof(buffer) - 1, &bytesRead, nullptr)) {
			...
			DWORD bytesWritten;
			if (WriteFile(hPipe, reply.c_str(), static_cast<DWORD>(reply.length()), &bytesWritten, nullptr)) {
			}
			else {
	...
	// END hook terminal into main loop
...
// Terminal addition code block BEGIN
...
void killTerminal(PROCESS_INFORMATION& pi)
{
	if (pi.hProcess != nullptr && pi.hProcess != INVALID_HANDLE_VALUE) {
		...
		if (TerminateProcess(pi.hProcess, 0))
		{
			...
			{
				pi.hProcess = nullptr;  // Set the handle to nullptr after closing
			}
			else {
				// Handle error if CloseHandle fails
				std::cout << "Failed to close the process handle. Error code: " << GetLastError() << std::endl;
			}

			if (CloseHandle(pi.hThread))
			{
				pi.hThread = nullptr;  // Set the handle to nullptr after closing
			}
			else {
				// Handle error if CloseHandle fails
				std::cout << "Failed to close the thread handle. Error code: " << GetLastError() << std::endl;
			}
		}
		else
		{
			// Handle error if TerminateProcess fails
			std::cout << "Failed to terminate the process. Error code: " << GetLastError() << std::endl;
		}
	}
...
void terminalCleanup(bool& isConnected, HANDLE& hPipe)
{
	...
	//	{
	//		hPipe = nullptr;  // Set the handle to nullptr after closing
	//	}
	//	else {
	//		// Handle error if CloseHandle fails
	//		std::cout << "Failed to close the pipe handle. Error code: " << GetLastError() << std::endl;
	//	}
	//}
...
// END terminal addition code block
